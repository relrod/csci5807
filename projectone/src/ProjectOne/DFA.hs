...
newStep :: DFA Int -> [CharOrClass] -> DFA (S.Set Int) -> DFA (S.Set Int)
...
newMoves :: DFA Int -> S.Set Int -> [CharOrClass] -> DFA (S.Set Int) -> DFA (S.Set Int)
newMoves _ _ [] d = d
newMoves d@(DFA m) s (x:xs) d' = newMoves d s xs (newMove m s x d')
{-# INLINE newMoves #-}

newMove :: NFA Int -> S.Set Int -> CharOrClass -> DFA (S.Set Int) -> DFA (S.Set Int)
...
toDeterministic :: DFA Int -> [CharOrClass] -> DFA (S.Set Int)
